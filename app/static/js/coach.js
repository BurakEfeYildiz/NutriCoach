...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    coachPhotoInput?.addEventListener('change', async (e) => {
        ...
        try {
            ...
            ui.openPhotoReviewModal(analysis, file, (createdMeal) => {
                appendMessageBubble({
                    role: 'assistant',
                    content: `Fotoğraftan öğün kaydedildi: ${createdMeal.description || 'Öğün'} (${createdMeal.total_calories} kcal)`,
    ...
    async function sendMessage(retryPayload = null) {
        ...
        const clientRequestId = retryPayload ? retryPayload.clientRequestId : crypto.randomUUID();
        ...
        try {
            const result = await api.sendMessage(conversationId, text, clientRequestId);
            ...
            if (result.assistant_message) {
                ...
                appendMessageBubble({
                    role: 'assistant',
                    content: 'İşleminiz başarıyla kaydedildi ancak koç yanıtı oluşturulamadı.',
                    created_at: new Date().toISOString(),
                    status: 'completed',
                }, result.user_message);
            }
            ...
            renderErrorBubble('Bağlantı kurulamadı. Tekrar deneyebilirsiniz.', pendingRetry);
            scrollToBottom();
        }
    }
    ...
    function appendMessageBubble(msg, userMsgContext = null) {
        ...
        if (actions && actions.length > 0) {
            actions.forEach(act => {
                ...
                pill.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg><span>${label}</span>`;
        ...
        const sources = msg.grounding_sources || [];
        if (sources && sources.length > 0) {
            ...
            groundCard.innerHTML = `
                <div class="grounding-badge">
                    ...
                    <span>Web ile doğrulandı</span>
                </div>
                <div class="grounding-links">
                    ${sources.map(s => `<a href="${encodeURI(s.url)}" target="_blank" rel="noopener noreferrer" class="grounding-link">${escapeHtml(s.title || 'Kaynak')} &nearr;</a>`).join('')}
                </div>
            `;
    ...
    function renderErrorBubble(errorText, retryData) {
        ...
        bubble.style.background = 'var(--danger-soft)';
